...
// Mapping matrix for Geo-Detection
const geoMap: Record<string, { lang: LanguageCode; currency: { code: string; symbol: string } }> = {
  ...
  'CN': { lang: 'ZH', currency: { code: 'EUR', symbol: '€' } }, // Using EUR as fallback if CNY not in main list
};
...
export const LanguageProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Wrapper to persist choice
  const setLanguage = (lang: LanguageCode) => {
    setLanguageState(lang);
    localStorage.setItem('eb_pref_lang', lang);
  };

  const setCurrency = (curr: { code: string; symbol: string }) => {
  ...
  useEffect(() => {
    const initSettings = async () => {
      ...
      // 2. Automated Detection based on IP
      try {
  ...
  return (
    <LanguageContext.Provider value={{ language, setLanguage, currency, setCurrency, t }}>
      {/* Hide content until initialized to prevent hydration mismatch/flicker */}
