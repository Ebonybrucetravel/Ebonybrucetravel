...
export async function publicRequest<T = any>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    if (options.headers) {
        Object.entries(options.headers).forEach(([k, v]) => { if (v !== undefined)
...
async function request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    try {
        ...
        if (error.message?.includes('Failed to fetch') || error.message?.includes('NetworkError')) {
            throw new ApiError('Cannot connect to server. Check your internet or try again later.', 0, 'NETWORK_ERROR');
        }
        throw new ApiError(error.message || 'An unexpected error occurred', error.status || 0, 'UNKNOWN_ERROR');
    }
    finally {
...
export const searchHotelsAmadeus = async (searchParams: HotelSearchParams): Promise<HotelSearchResponse> => {
    try {
        console.log('🏨 Starting hotel search with params:', searchParams);
        const response = await request<HotelSearchResponse>('/api/v1/bookings/search/hotels/amadeus', {
            method: 'POST',
            body: JSON.stringify({
...
export async function searchHotelsWithPagination(params: HotelSearchParams & {
    minPrice?: number;
    maxPrice?: number;
    ratings?: number[];
    amenities?: string[];
    ...
    try {
        ...
        throw new ApiError(error.message || 'Hotel search failed', error.status || 500, 'HOTEL_SEARCH_ERROR');
    }
}
export function transformHotelToSearchResult(hotel: HotelOffer, location: string, checkInDate: string, checkOutDate: string, index: number): any {
...
function extractAmenitiesFromDescription(description: string, chainCode?: string): string[] {
    ...
    if (amenities.length < 4) {
        amenities.push('Air Conditioning', 'TV', 'Private Bathroom', 'Hair Dryer', 'Coffee/Tea Maker', 'Safe', 'Desk');
    }
...
function getHotelImage(chainCode?: string, index: number = 0): string {
    ...
    if (chainCode) {
        const chainImages: Record<string, string> = {
            'MC': 'https://images.unsplash.com/photo-1611892440504-42a792e24d32?auto=format&fit=crop&q=80&w=400',
            'HI': 'https://images.unsplash.com/photo-1571003123894-1f0594d2b5d9?auto=format&fit=crop&q=80&w=400',
            'HS': 'https://images.unsplash.com/photo-1584132967334-10e028bd69f7?auto=format&fit=crop&q=80&w=400',
        };
...
export async function formatHotelSearchParams(location: string, checkInDate?: string, checkOutDate?: string, guests?: number, rooms?: number): Promise<HotelSearchParams> {
...
export async function searchAndTransformHotels(searchParams: HotelSearchParams, location: string): Promise<{
    success: boolean;
    results: any[];
    message?: string;
    total: number;
    ...
    try {
        ...
        const transformedResults = hotels.map((hotel, index) => transformHotelToSearchResult(hotel, location, searchParams.checkInDate, searchParams.checkOutDate, index));
        return {
            success: true,
...
function isValidExpiryDate(expiryDate: string): boolean {
    if (!expiryDate || !/^\d{4}-\d{2}$/.test(expiryDate))
        return false;
...
export const searchFlightsFixed = async (params: FlightSearchParams): Promise<FlightSearchResponse> => {
    try {
        ...
        throw new ApiError(error.message || 'Failed to search for flights', error.status || 500, 'FLIGHT_SEARCH_ERROR');
    }
};
export async function searchFlightsWithPagination(params: FlightSearchParams, maxPages: number = 3): Promise<{
    success: boolean;
    ...
    try {
        ...
        if (!searchResult.success) {
            throw new ApiError(searchResult.message || 'Flight search failed', 400, 'SEARCH_FAILED', searchResult);
        }
        ...
        if (!offerRequestId) {
            ...
            if (altId) {
                ...
                throw new ApiError('Flight search completed but no request ID returned. Please try again.', 500, 'NO_REQUEST_ID');
            }
        }
        ...
        if (offerRequestId) {
            do {
                ...
                try {
                    const offersResult = await bookingApi.getOffers(offerRequestId, nextCursor || undefined, 20, page === 1 ? 'total_amount' : undefined, 'asc');
        ...
        throw new ApiError(error.message || 'Flight search failed', error.status || 500, 'FLIGHT_SEARCH_ERROR');
    }
}
export const searchCarRentals = async (searchParams: CarRentalSearchParams): Promise<CarRentalSearchResponse> => {
    try {
        console.log('🚗 Starting car rental search with params:', searchParams);
        const response = await request<CarRentalSearchResponse>('/api/v1/bookings/search/car-rentals', {
            method: 'POST',
            body: JSON.stringify({
...
export async function searchCarRentalsWithPagination(params: CarRentalSearchParams & {
    minPrice?: number;
    maxPrice?: number;
    vehicleTypes?: string[];
    providers?: string[];
    ...
    try {
        ...
        if (allCars.length > 0) {
            ...
            if (params.vehicleTypes && params.vehicleTypes.length > 0) {
                allCars = allCars.filter(car => {
                    ...
                    return params.vehicleTypes!.some(type => vehicleType.includes(type.toLowerCase()) ||
            ...
            if (params.providers && params.providers.length > 0) {
                allCars = allCars.filter(car => {
                    ...
                    return params.providers!.some(provider => providerName.includes(provider.toLowerCase()));
                });
            }
            if (params.sortBy) {
                allCars.sort((a, b) => {
        ...
        throw new ApiError(error.message || 'Car rental search failed', error.status || 500, 'CAR_RENTAL_SEARCH_ERROR');
    }
}
export function transformCarRentalToSearchResult(car: CarRentalOffer | null | undefined, pickupLocation: string, dropoffLocation: string, index: number): any {
    if (!car) {
        return {
            id: `car-${index}`,
    ...
    return {
        ...
        realData: {
            ...
            cancellationRules: carInfo.cancellationRules || [],
            providerName: carInfo.serviceProvider?.name,
            providerCode: carInfo.serviceProvider?.code,
            serviceProvider: carInfo.serviceProvider ? { name: carInfo.serviceProvider.name, logoUrl: carInfo.serviceProvider.logoUrl, termsUrl: carInfo.serviceProvider.termsUrl } : undefined,
            vehicleImageURL: carInfo.vehicle?.imageURL,
...
function extractCarAmenities(category?: string, providerName?: string): string[] {
    ...
    amenities.push('Unlimited Mileage', 'Full Insurance', '24/7 Customer Support', 'Free Cancellation', 'Child Seats Available');
    return Array.from(new Set(amenities));
}
...
function getCarImage(vehicleCode?: string, category?: string, index: number = 0): string {
    const carImages = [
        ...
        'https://images.unsplash.com/photo-1563720223486-3294265d5a7c?auto=format&fit=crop&q=80&w=400',
    ];
...
export async function formatCarRentalSearchParams(pickupLocation: string, dropoffLocation: string, pickupDate?: string, dropoffDate?: string, pickupTime?: string, dropoffTime?: string, passengers?: number): Promise<CarRentalSearchParams> {
...
export async function searchAndTransformCarRentals(searchParams: CarRentalSearchParams, pickupLocation: string, dropoffLocation: string): Promise<{
    success: boolean;
    results: any[];
    message?: string;
    total: number;
    ...
    try {
        ...
        const transformedResults = cars.map((car, index) => transformCarRentalToSearchResult(car, pickupLocation, dropoffLocation, index));
        return {
            success: true,
            results: transformedResults,
            message: response.message,
...
export const createCarRentalBooking = async (bookingData: CarRentalBookingRequest): Promise<CarRentalBookingResponse> => {
    try {
        console.log('🚗 Creating car rental booking...');
        const validation = validateCarRentalBookingData(bookingData);
        if (!validation.isValid) {
            throw new ApiError(`Booking validation failed: ${validation.errors.join(', ')}`, 400, 'BOOKING_VALIDATION_FAILED');
        }
        ...
        const response = await request<CarRentalBookingResponse>('/api/v1/bookings', {
            method: 'POST',
        ...
        throw new ApiError(error.message || 'Failed to create car rental booking', error.status || 500, 'CAR_RENTAL_BOOKING_ERROR');
    }
};
export const cancelCarRentalBooking = async (bookingId: string): Promise<CarRentalCancellationResponse> => {
    try {
        console.log(`🚗 Cancelling car rental booking: ${bookingId}`);
        const response = await request<CarRentalCancellationResponse>(`/api/v1/bookings/car-rentals/bookings/${bookingId}/cancel`, {
            method: 'POST',
        });
        ...
        throw new ApiError(error.message || 'Failed to cancel car rental booking', error.status || 500, 'CAR_RENTAL_CANCELLATION_ERROR');
    }
};
export const getCarRentalBooking = async (bookingId: string): Promise<CarRentalBookingResponse> => {
    try {
        console.log(`🚗 Fetching car rental booking: ${bookingId}`);
        const response = await request<CarRentalBookingResponse>(`/api/v1/bookings/car-rentals/bookings/${bookingId}`, {
            method: 'GET',
        });
        ...
        throw new ApiError(error.message || 'Failed to fetch car rental booking', error.status || 500, 'CAR_RENTAL_BOOKING_FETCH_ERROR');
    }
};
export async function createCompleteCarRentalBooking(offerId: string, carData: any, passengerInfo: {
    firstName: string;
    lastName: string;
    email: string;
    phone: string;
}, specialRequests?: string, flightNumber?: string): Promise<CarRentalBookingResponse> {
    try {
        ...
        if (!response.success) {
            throw new ApiError(response.message || 'Car rental booking failed', response.status || 500, 'CAR_RENTAL_BOOKING_FAILED');
        }
...
export const trackPaymentStatus = async (paymentIntentId: string, interval: number = 2000, maxAttempts: number = 30): Promise<{
    status: string;
    succeeded: boolean;
    error?: string;
}> => {
...
export const processBookingPayment = async (bookingType: 'flights' | 'hotels' | 'car-rentals', bookingData: any, userInfo: {
    firstName: string;
    lastName: string;
    email: string;
    phone: string;
    [key: string]: any;
}, isAuthenticated: boolean = false): Promise<{
    success: boolean;
    paymentIntent?: any;
    clientSecret?: string;
    error?: string;
...
export const createBookingAfterPayment = async (bookingType: 'flights' | 'hotels' | 'car-rentals', selectedResult: any, userInfo: any, paymentIntentId: string, specialRequests?: string): Promise<{
    success: boolean;
    bookingId?: string;
    message?: string;
    error?: string;
...
async function createFlightBookingAfterPayment(flightData: any, userInfo: any, paymentIntentId: string) {
...
async function createHotelBookingAfterPayment(hotelData: any, userInfo: any, paymentIntentId: string, specialRequests?: string) {
...
async function createCarRentalBookingAfterPayment(carData: any, userInfo: any, paymentIntentId: string, specialRequests?: string) {
...
export const completeBookingWithPayment = async (bookingType: 'flights' | 'hotels' | 'car-rentals', selectedResult: any, userInfo: {
    firstName: string;
    lastName: string;
    email: string;
    phone: string;
    [key: string]: any;
}, stripe: any, elements: any, isAuthenticated: boolean = false, specialRequests?: string): Promise<{
    success: boolean;
    bookingId?: string;
    message?: string;
    error?: string;
    ...
    try {
        ...
        const paymentResult = await processBookingPayment(bookingType, selectedResult, userInfo, isAuthenticated);
        ...
        const bookingResult = await createBookingAfterPayment(bookingType, selectedResult, userInfo, stripeResult.paymentIntent.id, specialRequests);
...
export const bookingApi = {
    ...
    getOffers: (offerRequestId: string, cursor?: string, limit: number = 20, sort?: 'total_amount' | 'total_duration', sortOrder: 'asc' | 'desc' = 'asc') => {
        const params = new URLSearchParams({
    ...
    getPublicBookingByReference: (reference: string) => {
        return publicRequest<any>(`/api/v1/bookings/public/by-reference/${encodeURIComponent(reference)}`, { method: 'GET' });
    },
    getPublicBookingById: (bookingId: string, reference: string, email: string) => {
        const params = new URLSearchParams({ reference, email });
        return publicRequest<any>(`/api/v1/bookings/public/by-id/${encodeURIComponent(bookingId)}?${params}`, { method: 'GET' });
    },
    ...
    requestCancelAmadeusHotel: (bookingId: string) => {
        return request<{
            ...
            message?: string;
        }>(`/api/v1/bookings/hotels/bookings/${encodeURIComponent(bookingId)}/request-cancel`, { method: 'POST' });
    },
...
export const paymentApi = {
    ...
    getHotelImages: (hotelId: string, params?: {
        hotelName?: string;
        ...
        if (params?.googlePlaceId)
            search.set('googlePlaceId', params.googlePlaceId);
        ...
        return request<{
            ...
            message?: string;
        }>(`/api/v1/bookings/hotels/${encodeURIComponent(hotelId)}/images${query ? `?${query}` : ''}`, { method: 'GET' });
    },
};
export const hotelApi = {
    ...
    validateHotelBooking: async (hotelId: string, offerId: string, checkInDate: string, checkOutDate: string, guests: number, rooms: number) => {
        try {
...
export async function handleResetPassword(token: string, password: string): Promise<{
    success: boolean;
    message: string;
    user?: User;
}> {
...
export async function handleVerifyEmail(token: string): Promise<{
    success: boolean;
    message: string;
    user?: User;
    verified?: boolean;
...
export async function createAmadeusHotelBooking(offerId: string, hotelData: any, guestInfo: {
    firstName: string;
    lastName: string;
    email: string;
    phone: string;
    ...
    try {
        ...
        if (!response.success) {
            throw new ApiError(response.message || 'Hotel booking failed', response.status || 500, 'HOTEL_BOOKING_FAILED');
        }
        ...
        throw new ApiError(error.message || 'Failed to create hotel booking', error.status || 500, 'HOTEL_BOOKING_ERROR');
    }
}
export function getVendorCodeFromCardNumber(cardNumber: string): string {
    ...
    if (/^3[47]/.test(cleanNumber))
        return 'AX';
    if (/^6(011|4[4-9]|5)/.test(cleanNumber))
        return 'DS';
    if (/^3(0[0-5]|6|8)/.test(cleanNumber))
        return 'DC';
    if (/^35(2[8-9]|[3-8][0-9])/.test(cleanNumber))
        return 'JC';
    return 'VI';
}
export function transformFlightOfferToSearchResult(offer: any, index: number, isReturnFlight: boolean = false): any {
    try {
