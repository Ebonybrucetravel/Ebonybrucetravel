...
export const formatPrice = (amount: number, currency = 'GBP') =>
  `${currencySymbol(currency)}${amount.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
...
export const getProductMeta = (
  itemType?: string,
): { productType: ProductType; provider: Provider } => {
  switch (itemType) {
    case 'hotels':
