...
export interface WakanowSelectResponse {
  ...
  data: {
    ...
    message: string;
    stop_information?: any | null;
    stopInformation?: any | null;
    wakanowSelectData?: string | null;
...
const BACKEND_BASE_URL =
  ...
  'https://ebony-bruce-production.up.railway.app/api/v1';
const WAKANOW_BASE_URL = `${BACKEND_BASE_URL}/bookings/wakanow`;

/**
 * Low-level fetch wrapper for the Wakanow backend.
 *
 * IMPORTANT: this function only converts backend errors to `SELECTION_EXPIRED`
 * when the backend explicitly says the selection has expired. All other
 * failures (validation, server errors, network issues) are surfaced with
 * their real message so the caller can react appropriately.
 */
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (isJson && data) {
        ...
        // Only treat as expired when the backend says so explicitly.
        if (
          errorLower.includes('expired') ||
          errorLower.includes('please search for flights again') ||
          errorLower.includes('selection has expired')
        ) {
          isExpired = true;
          errorMessage = 'SELECTION_EXPIRED';
        }

        // Wakanow's own "Bad Request" from its /select endpoint.
        if (
          response.status === 400 &&
          (data?.Message === 'Bad Request' || data?.message === 'Bad Request')
        ) {
          isExpired = true;
          errorMessage = 'SELECTION_EXPIRED';
        }
        ...
        if (responseText && responseText.length > 0) {
          ...
          if (
            ...
            textLower.includes('please search for flights again')
          ) {
            isExpired = true;
            errorMessage = 'SELECTION_EXPIRED';
          }
          ...
          console.error(
            '📦 Empty error response with status:',
            response.status,
          );
        }
      }
    ...
    if (data && typeof data === 'object') {
      if (data.success === false) {
        const errorMsg =
          data.message || data.error || 'Backend operation failed';
...
export function formatWakanowTitle(title: string | undefined): string {
  ...
  if (titleMap[lowerTitle]) return titleMap[lowerTitle];

  return 'Mr';
}

export function formatWakanowGender(
...
export function createWakanowPassenger(p: any): WakanowPassenger {
  return {
    ...
    PassportIssuingAuthority:
      p.PassportIssuingAuthority || p.passportIssuingAuthority || '',
    PassportIssueCountryCode:
      p.PassportIssueCountryCode || p.passportIssueCountry || '',
...
export async function getWakanowAirports(
  query?: string,
  limit?: number,
): Promise<WakanowAirport[]> {
  const now = Date.now();
  if (
    cachedAirports &&
    airportCacheTime &&
    now - airportCacheTime < AIRPORT_CACHE_TTL
  ) {
    if (query) {
  ...
  try {
    ...
    console.error(
      'Failed to fetch airports from backend, using fallback:',
      error,
    );
    ...
    const fallbackAirports: WakanowAirport[] = [
      ...
      },
      {
        ...
        CityCountry: 'Port Harcourt, Nigeria',
        City: 'Port Harcourt',
        Country: 'Nigeria',
      },
      {
...
export async function searchWakanowFlights(
  params: WakanowFlightSearchParams,
): Promise<WakanowSearchResponse> {
  console.log('🔍 Search flights via backend:', params);

  const formattedItineraries = params.Itineraries.map(
    (itin: {
      Departure: string;
      Destination: string;
      DepartureDate: string;
  ...
  const backendParams = {
    flightSearchType:
      params.FlightSearchType === 'Oneway'
        ? 'Oneway'
        : params.FlightSearchType === 'Return'
          ? 'Return'
          : 'Multidestination',
...
export async function selectWakanowFlight(
  selectData: string,
  targetCurrency: string = 'NGN',
): Promise<WakanowSelectResponse> {
  ...
  const cleanedSelectData = selectData.trim();

  if (cleanedSelectData.length < 10) {
    console.error('❌ Invalid selectData (too short):', cleanedSelectData);
    throw new Error(
      'Invalid selectData (too short). Please search for flights again.',
    );
  }
  ...
  // Enrich response so the long blob is always accessible via `longToken`.
  const rawData: any = response.data;
  const longBlob =
    rawData?.wakanowSelectData || rawData?.bookingData || rawData?.longToken;
...
export async function bookWakanowFlightGuest(bookingData: {
  bookingId: string;
  selectData: string;
  targetCurrency?: string;
  passengers: WakanowBackendPassenger[];
...
export async function ticketWakanowFlight(
  ...
  console.log('🎫 Issue ticket via backend:', {
    bookingId,
    pnrNumber,
    localBookingId,
  });
...
export async function getWakanowWalletBalance(
  authToken?: string,
): Promise<WakanowWalletBalanceResponse> {
  console.log('💰 Get wallet balance via backend');

  return backendFetch<WakanowWalletBalanceResponse>('/wallet-balance', {
    method: 'GET',
    authToken,
  });
}
...
export async function healthCheck(): Promise<HealthCheckResponse> {
  ...
  return backendFetch<HealthCheckResponse>('/health', {
    method: 'GET',
  });
}
...
export function useWakanowSelect() {
  ...
  const [selectedData, setSelectedData] = useState<
    WakanowSelectResponse['data'] | null
  >(null);
  const [fullResponse, setFullResponse] =
    useState<WakanowSelectResponse | null>(null);

  const select = useCallback(
    async (selectDataParam: string, targetCurrency?: string) => {
      setLoading(true);
      setError(null);

      try {
        const response = await selectWakanowFlight(
          selectDataParam,
          targetCurrency,
        );
...
export function useWakanowBooking() {
  ...
  const [booking, setBooking] = useState<
    WakanowBookingResponse['data'] | null
  >(null);
...
export function useWakanowTicket() {
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [ticket, setTicket] = useState<WakanowTicketResponse['data'] | null>(
    null,
  );
  ...
  const issue = useCallback(
    async (
      ...
      try {
        const response = await ticketWakanowFlight(
          bookingId,
          pnrNumber,
          localBookingId,
          authToken,
