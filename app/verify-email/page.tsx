...
export default function VerifyEmailPage() {
    ...
    if (status === 'loading') {
        return (<div className="min-h-screen bg-[#f8fbfe] flex items-center justify-center p-6">
    ...
    return (<div className="min-h-screen bg-[#f8fbfe] flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 max-w-md w-full text-center">
        {status === 'success' ? (<>
            ...
            <button onClick={() => router.push('/login')} className="w-full bg-[#33a8da] text-white font-bold py-3 rounded-xl hover:bg-[#2c98c7] transition">
              Sign in
            </button>
          </>) : (<>
            ...
            <p className="text-gray-600 text-sm mb-6">{message}</p>
            <button onClick={() => router.push('/login')} className="w-full bg-gray-900 text-white font-bold py-3 rounded-xl hover:bg-black transition">
              Back to Sign in
            </button>
          </>)}
      </div>
