  ...
  async create(booking: Partial<Booking>): Promise<Booking> {
    const created = await this.prisma.booking.create({
      data: {
        ...
        ...(booking.cancellationDeadline && { cancellationDeadline: booking.cancellationDeadline }),
        ...(booking.cancellationPolicySnapshot !== undefined && { cancellationPolicySnapshot: booking.cancellationPolicySnapshot }),
        ...(booking.clientIp && { clientIp: booking.clientIp }),
        ...(booking.userAgent && { userAgent: booking.userAgent }),
        ...(booking.policyAcceptedAt && { policyAcceptedAt: booking.policyAcceptedAt }),
        ...(booking.stripeChargeId && { stripeChargeId: booking.stripeChargeId }),
        ...(booking.confirmationEmailSentAt && { confirmationEmailSentAt: booking.confirmationEmailSentAt }),
      },
      include: {
        user: {
          select: {
  ...
  async update(id: string, data: Partial<Booking>): Promise<Booking> {
    const updated = await this.prisma.booking.update({
      ...
      data: {
        ...
        ...(data.cancellationDeadline !== undefined && { cancellationDeadline: data.cancellationDeadline }),
        ...(data.cancellationPolicySnapshot !== undefined && { cancellationPolicySnapshot: data.cancellationPolicySnapshot }),
        ...(data.clientIp !== undefined && { clientIp: data.clientIp }),
        ...(data.userAgent !== undefined && { userAgent: data.userAgent }),
        ...(data.policyAcceptedAt !== undefined && { policyAcceptedAt: data.policyAcceptedAt }),
        ...(data.stripeChargeId !== undefined && { stripeChargeId: data.stripeChargeId }),
        ...(data.confirmationEmailSentAt !== undefined && { confirmationEmailSentAt: data.confirmationEmailSentAt }),
      },
      include: {
        user: {
          select: {
