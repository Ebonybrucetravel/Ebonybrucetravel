...
export class CreateGuestBookingUseCase {
  ...
  async execute(dto: CreateGuestBookingDto): Promise<Booking> {
    ...
    if (dto.provider === Provider.AMADEUS && dto.productType === 'CAR_RENTAL') {
      ...
      const booking = await this.bookingService.createBooking({
        ...
        bookingData: {
          ...
          // ✅ Store paymentMethodId for PCI compliance
    ...
    // ==================== HOTEL BOOKING (ADD PAYMENT METHOD ID) ====================
    // If this is a hotel booking, store paymentMethodId
    if (dto.productType === 'HOTEL' && (dto.provider === Provider.AMADEUS || dto.provider === Provider.HOTELBEDS)) {
      const paymentMethodId = dto.paymentMethodId || dto.bookingData?.paymentMethodId;
      if (paymentMethodId) {
        cleanedBookingData = {
          ...cleanedBookingData,
