...
export class ChargeAmadeusCarRentalMarginUseCase {
  ...
  async execute(bookingId: string, userId: string): Promise<{ booking: any; paymentIntentId: string }> {
    ...
    if (booking.provider !== Provider.AMADEUS || booking.productType !== 'CAR_RENTAL') {
      throw new BadRequestException('This endpoint is only for Amadeus car rental bookings');
    }
    ...
    if (!bookingData?.payment_card_info?.encrypted) {
      throw new BadRequestException(
        'Booking has no stored card. Create the booking with a payment card first (POST /bookings/car-rentals/bookings or /bookings/car-rentals/bookings/guest).',
      );
    }
    ...
    try {
      ...
      this.logger.error(
        `Failed to decrypt card for booking ${bookingId}: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    ...
    // 1) Create Amadeus transfer order (uses and clears stored card)
    await this.createCarRentalBookingUseCase.createAmadeusOrderAfterPayment(bookingId);

    // 2) Charge margin via Stripe with the card we kept in memory
  ...
  private async sendBookingEmails(
    ...
    try {
      ...
      if (driver) {
        bookingDetails.driverName = [driver.title, driver.firstName, driver.lastName].filter(Boolean).join(' ');
      }

      await this.resendService.sendBookingConfirmationEmail({
        to: user.email,
        customerName: user.name || passengerInfo?.firstName || driver?.firstName || 'Valued Customer',
        bookingReference: booking.reference,
        productType: booking.productType,
        provider: booking.provider,
        bookingDetails,
