...
const TrendingDestinations: React.FC<TrendingDestinationsProps> = ({ onCityClick }) => {
    ...
    return (<section className="px-4 md:px-8 lg:px-16 py-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {destinations.map((destination) => (<div key={destination.id} className="group cursor-pointer rounded-xl overflow-hidden bg-white shadow-md hover:shadow-xl transition-all duration-300" onClick={() => handleCityClick({ code: destination.code, name: destination.city })}>
            <div className="relative h-48 overflow-hidden">
              <img src={destination.image} alt={`${destination.city}, ${destination.country}`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy"/>
            ...
            <div className="p-4">
              ...
              <button className="mt-3 w-full py-2 text-sm font-semibold rounded-lg transition-all duration-300" style={{
