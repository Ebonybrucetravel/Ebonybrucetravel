...
const ReviewTrip: React.FC<ReviewTripProps> = ({ item, searchParams, onBack, isLoggedIn, user, isCreating, onProceedToPayment, onSignInRequired, }) => {
    useLanguage();
    const offerCurrency = (item?.realData?.currency ?? 'GBP').toUpperCase();
    if (!item) {
        return (<div className="bg-[#f8fbfe] min-h-screen py-12">
    ...
    const handleCompleteBooking = async () => {
        ...
        setIsBooking(true);
        try {
            await onProceedToPayment({ firstName, lastName, email, phone }, voucherApplied?.valid ? voucherCode.trim() : undefined);
        }
        catch (error: any) {
          ...
          <div className="flex-1 space-y-8">
            ...
            <section className="bg-white rounded-[32px] p-10 shadow-xl border border-gray-100">
              <div className="flex items-center justify-between mb-10">
                ...
                {isLoggedIn && user && (<span className="text-[10px] font-black text-green-500 bg-green-50 px-3 py-1 rounded-full uppercase tracking-widest">
                    ✓ From your profile
                  </span>)}
              </div>

              {isLoggedIn && user ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {!isLoggedIn && (<div className="mt-6 p-4 bg-blue-50 rounded-xl flex items-center gap-3">
            ...
            {isAmadeusHotel && (<section className="bg-white rounded-[32px] p-10 shadow-xl border border-gray-100">
                <h2 className="text-2xl font-black text-[#001f3f] mb-6 uppercase">Cancellation and no-show</h2>
                <div className="space-y-4 text-sm">
                  <p className="font-bold text-gray-900">{cancellationPolicyText}</p>
                  {cancellationDeadlineDisplay ? (<p className="font-bold text-gray-700">
                ...
                <label className="mt-6 flex items-start gap-3 cursor-pointer group">
                  <input type="checkbox" checked={policyAccepted} onChange={(e) => setPolicyAccepted(e.target.checked)} className="mt-1 w-5 h-5 rounded border-2 border-gray-300 text-[#33a8da] focus:ring-[#33a8da]"/>
          ...
          <aside className="w-full lg:w-[460px]">
            <div className="bg-white rounded-[32px] shadow-2xl p-10 sticky top-24 border border-gray-100">
              ...
              <div className="space-y-6 mb-10">
                 ...
                 {markupAmount > 0 && (<div className="flex justify-between items-center">
                 ...
                 <div className="flex justify-between items-center">
                    ...
                    {isAmadeusHotel ? (<span className="text-[10px] font-black text-gray-500 uppercase">Included</span>) : serviceFeeAmount > 0 ? (<span className="text-sm font-bold text-gray-900">{formatPrice(serviceFeeAmount, offerCurrency)}</span>) : (<span className="text-[10px] font-black text-green-500 uppercase">Waived</span>)}
                 </div>
                 <div className="pt-2 border-t border-gray-100">
                   ...
                   <div className="flex gap-2">
                     <input type="text" value={voucherCode} onChange={(e) => setVoucherCode(e.target.value.toUpperCase())} placeholder="EBT-V-ABC123" className="flex-1 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#33a8da]/20 focus:border-[#33a8da]"/>
                     <button type="button" disabled={isValidatingVoucher || !voucherCode.trim()} onClick={handleApplyVoucher} className="px-4 py-2.5 bg-[#33a8da] text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-[#2c98c7] disabled:opacity-50">
                       {isValidatingVoucher ? 'Checking…' : 'Apply'}
                     </button>
                   </div>
                   {voucherError && (<p className="mt-2 text-[11px] text-red-500 font-bold">{voucherError}</p>)}
                   {voucherApplied?.valid && (<p className="mt-2 text-[11px] text-green-600 font-bold">
              ...
              <button onClick={handleCompleteBooking} disabled={isBooking || isCreating || (isAmadeusHotel && !policyAccepted)} className="w-full bg-[#33a8da] text-white font-black py-6 rounded-2xl shadow-xl hover:bg-[#2c98c7] transition transform active:scale-95 disabled:opacity-50 uppercase tracking-widest text-sm">
