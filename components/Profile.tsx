      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <main className="flex-1 space-y-8" ref={contentTopRef}>
            ...
            {activeTab === 'payment' && (
              <div className="animate-in fade-in duration-500 space-y-8">
                <div className="bg-white rounded-[24px] p-8 md:p-10 shadow-sm border border-gray-100/50"><h1 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Payment Methods</h1><p className="text-gray-400 font-bold text-sm mt-1">Stored cards for faster checkout.</p></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div onClick={() => !isSavingCard && setShowAddPaymentForm(!showAddPaymentForm)} className={`bg-white rounded-[32px] p-8 border-2 ${showAddPaymentForm ? 'border-[#33a8da]' : 'border-dashed border-gray-100'} flex flex-col items-center justify-center text-center group hover:border-[#33a8da]/50 transition cursor-pointer h-[200px]`}>
                    <div className={`w-12 h-12 ${showAddPaymentForm ? 'bg-blue-50 text-[#33a8da]' : 'bg-gray-50 text-gray-300'} rounded-full flex items-center justify-center mb-4 transition`}><svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path d={showAddPaymentForm ? "M18 12H6" : "M12 4v16m8-8H4"} /></svg></div>
                  ...
                  {showAddPaymentForm && (
                    <div className="bg-white rounded-[32px] p-8 border border-[#33a8da] shadow-sm animate-in slide-in-from-right duration-300">
                      <form onSubmit={handleAddPayment} className="space-y-4">
                        <input required type="text" value={newCardData.holderName} onChange={(e) => setNewCardData({...newCardData, holderName: e.target.value.toUpperCase()})} className="w-full px-4 py-2.5 bg-gray-50 rounded-xl font-bold" placeholder="CARDHOLDER NAME" />
                        <input required type="text" maxLength={19} value={newCardData.number} onChange={(e) => setNewCardData({...newCardData, number: formatCardNumber(e.target.value)})} className="w-full px-4 py-2.5 bg-gray-50 rounded-xl font-bold" placeholder="CARD NUMBER" />
                        <div className="grid grid-cols-2 gap-4">
                          <input required type="text" maxLength={5} value={newCardData.expiry} onChange={(e) => { let v = e.target.value.replace(/\D/g, ''); if (v.length >= 2) v = v.substring(0, 2) + '/' + v.substring(2, 4); setNewCardData({...newCardData, expiry: v}); }} className="w-full px-4 py-2.5 bg-gray-50 rounded-xl font-bold" placeholder="MM/YY" />
                          <input required type="password" maxLength={4} value={newCardData.cvv} onChange={(e) => setNewCardData({...newCardData, cvv: e.target.value.replace(/\D/g, '')})} className="w-full px-4 py-2.5 bg-gray-50 rounded-xl font-bold" placeholder="CVV" />
                        </div>
                        <button type="submit" disabled={isSavingCard} className="w-full bg-[#33a8da] text-white py-3 rounded-xl font-bold uppercase text-xs tracking-widest">{isSavingCard ? 'Saving...' : 'Save Card'}</button>
                      </form>
                    </div>
                  )}
                </div>
                <div className="space-y-4">{savedCards.map(card => (<div key={card.id} className="flex items-center justify-between p-6 bg-white rounded-2xl border border-gray-100"><div className="flex items-center gap-6"><img src={card.icon} className="h-8 w-auto grayscale opacity-50" alt="" /><div><p className="font-bold">•••• •••• {card.last4}</p><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{card.expiry}</p></div></div><button onClick={() => handleRemoveCard(card.id)} className="text-[10px] font-bold text-red-400 hover:text-red-600 uppercase tracking-widest transition">Remove</button></div>))}</div>
              </div>
            )}
            ...
            {activeTab === 'saved' && (
              <div className="animate-in fade-in duration-500 space-y-8">
                <div className="flex justify-between items-center">
                  <div><h1 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Saved Items</h1><p className="text-gray-400 font-bold text-sm mt-1">Your personal travel wishlist.</p></div>
                ...
                <div className="space-y-4">
                  {savedItems.length > 0 ? savedItems.map(renderSavedCard) : (
                    <div className="bg-white rounded-[32px] p-16 text-center border-4 border-dashed border-gray-100"><h3 className="text-xl font-bold text-gray-900 mb-2">No saved items</h3><p className="text-gray-400 font-bold">Items you "love" will appear here.</p></div>
                  )}
                </div>
              </div>
            )}
            ...
            {activeTab === 'rewards' && (
              <div className="animate-in fade-in duration-500 space-y-8">
                <div><h1 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Rewards</h1><p className="text-gray-400 font-bold text-sm mt-1">Track your loyalty points.</p></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="bg-white rounded-[24px] p-8 border border-gray-100 shadow-sm"><p className="text-[10px] text-gray-400 font-bold uppercase mb-1">Current Balance</p><p className="text-5xl font-black text-gray-900">45,000 <span className="text-sm font-bold text-gray-400">pts</span></p></div>
                  <div className="bg-white rounded-[24px] p-8 border border-gray-100 shadow-sm"><h3 className="font-bold text-gray-900 uppercase text-xs mb-4">Quick Perks</h3><ul className="space-y-3 text-xs font-bold text-gray-600">{['Priority Check-in', 'Free Shuttle', '15% Off Hotels'].map(p => (<li key={p} className="flex items-center gap-2"><div className="w-1.5 h-1.5 bg-[#33a8da] rounded-full"></div>{p}</li>))}</ul></div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'security' && (
              <div className="animate-in fade-in duration-500 space-y-8">
                <div className="bg-white rounded-[24px] p-8 md:p-10 shadow-sm border border-gray-100/50"><h1 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Security</h1><p className="text-gray-400 font-bold text-sm mt-1">Update passwords and 2FA settings.</p></div>
                <div className="bg-white rounded-[32px] p-8 border border-gray-100">
                  <div className="space-y-6">
                    <input type="password" value={passwords.current} onChange={(e) => setPasswords({...passwords, current: e.target.value})} className="w-full px-5 py-3.5 bg-gray-50 rounded-xl font-bold" placeholder="Current Password" />
                    <input type="password" value={passwords.new} onChange={(e) => setPasswords({...passwords, new: e.target.value})} className="w-full px-5 py-3.5 bg-gray-50 rounded-xl font-bold" placeholder="New Password" />
                    <button onClick={handleUpdatePassword} disabled={isUpdatingPassword || !passwords.new} className="bg-[#33a8da] text-white px-8 py-3.5 rounded-xl font-bold uppercase text-xs tracking-widest">{isUpdatingPassword ? 'Updating...' : 'Change Password'}</button>
                  </div>
                </div>
                <div className="bg-white rounded-[32px] p-8 border border-gray-100 flex items-center justify-between"><p className="text-sm font-bold text-gray-600">Two-Factor Authentication</p><button onClick={() => !twoFactorEnabled ? open2FAModal() : setTwoFactorEnabled(false)} className={`px-6 py-2.5 rounded-xl font-bold text-[10px] uppercase tracking-widest ${twoFactorEnabled ? 'bg-red-50 text-red-500' : 'bg-blue-600 text-white'}`}>{twoFactorEnabled ? 'Disable' : 'Enable'}</button></div>
              </div>
            )}
              ...
              <div className="animate-in fade-in duration-500">
                ...
                <div className="mt-8 flex justify-end gap-5">
                  <button 
                    ...
                    className="px-10 py-3.5 bg-[#33a8da] text-white font-bold rounded-2xl shadow-xl shadow-blue-500/10 active:scale-95 text-lg"
                  >
                    Save Preferences
                  </button>
                </div>
            ...
            {activeTab === 'travelers' && (
              <div className="animate-in fade-in duration-500 space-y-8">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white rounded-[24px] p-8 md:p-10 shadow-sm border border-gray-100/50 gap-4"><div><h1 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Other Travelers</h1><p className="text-gray-400 font-bold text-sm mt-1">Quicker booking for family and friends.</p></div><button onClick={() => setShowAddTravelerForm(!showAddTravelerForm)} className="bg-[#33a8da] text-white px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg active:scale-95">{showAddTravelerForm ? 'Cancel' : 'Add Traveler'}</button></div>
                {showAddTravelerForm && (
                  <div className="bg-white rounded-[32px] p-8 border-2 border-[#33a8da] shadow-sm animate-in slide-in-from-top-4 duration-300">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <input type="text" value={newTraveler.name} onChange={(e) => setNewTraveler({...newTraveler, name: e.target.value})} className="w-full px-5 py-3.5 bg-gray-50 rounded-xl font-bold" placeholder="Full Name" />
                      <select value={newTraveler.relationship} onChange={(e) => setNewTraveler({...newTraveler, relationship: e.target.value})} className="w-full px-5 py-3.5 bg-gray-50 rounded-xl font-bold"><option>Spouse</option><option>Child</option><option>Friend</option></select>
                      <input type="date" value={newTraveler.dob} onChange={(e) => setNewTraveler({...newTraveler, dob: e.target.value})} className="w-full px-5 py-3.5 bg-gray-50 rounded-xl font-bold" />
                      <button onClick={handleAddTraveler} className="bg-[#33a8da] text-white py-3.5 rounded-xl font-bold uppercase text-xs">Confirm</button>
                    </div>
                  </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{travelers.map(t => (<div key={t.id} className="bg-white rounded-[24px] p-6 border border-gray-100 flex items-center justify-between hover:border-[#33a8da] transition"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center text-[#33a8da]"><svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg></div><div><h4 className="font-bold text-gray-900">{t.name}</h4><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t.relationship} • {t.dob}</p></div></div><button onClick={() => handleRemoveTraveler(t.id)} className="text-gray-300 hover:text-red-500 transition p-2"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button></div>))}</div>
              </div>
            )}
          </main>
        </div>
          ...
          <div className="bg-white w-full max-w-[440px] rounded-[32px] shadow-2xl overflow-hidden relative p-10 text-center">
            ...
            {faStep === 'otp' ? (
              <>
                ...
                <div className="flex justify-center gap-3 mb-10">{otp.map((digit, idx) => (<input key={idx} ref={el => { otpRefs.current[idx] = el; }} type="text" maxLength={1} value={digit} onChange={e => handleOtpChange(idx, e.target.value)} onKeyDown={e => handleOtpKeyDown(idx, e)} className="w-11 h-14 bg-gray-50 border-2 border-gray-100 rounded-xl text-center text-xl font-black focus:border-[#33a8da] outline-none" />))}</div>
