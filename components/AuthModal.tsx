...
const AuthModal: React.FC<AuthModalProps> = ({
  ...
  /** Switch between modes and update the URL */
    ...
    >
      <div className="w-full max-w-[780px] bg-white rounded-2xl shadow-2xl overflow-hidden flex animate-in zoom-in-95 duration-300 max-h-[90vh]">
        ...
        <div className="flex-1 p-8 relative overflow-y-auto">
          {onClose && (
            <button onClick={onClose} className="absolute top-4 right-4 p-1.5 text-gray-300 hover:text-gray-600 transition">
          ...
          <div className="mt-5 text-center text-xs text-gray-500">
            {mode === 'login' && (
              <p>Don&apos;t have an account? <button onClick={() => switchMode('register')} className="text-[#33a8da] font-bold hover:underline">Sign Up</button></p>
            )}
            {mode === 'register' && (
              <p>Already have an account? <button onClick={() => switchMode('login')} className="text-[#33a8da] font-bold hover:underline">Sign In</button></p>
            )}
            {(mode === 'forgot-password' || mode === 'reset-password') && (
              <button onClick={() => switchMode('login')} className="text-[#33a8da] font-bold hover:underline">← Back to Sign In</button>
            )}
          </div>
        </div>
      </div>
