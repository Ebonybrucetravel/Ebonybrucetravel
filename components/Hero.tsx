...
const Hero: React.FC<HeroProps> = ({ onSearch, loading, activeSearchTab = 'flights', onTabChange }) => {
    const { t } = useLanguage();
    ...
    const handleQuickCarSearch = (location: {
        name: string;
        code: string;
    }) => {
        const searchData = {
    ...
    return (<section className="relative min-h-[450px] md:min-h-[650px] flex items-center justify-center overflow-hidden py-4 md:py-6">
      <div className="absolute inset-0 z-0">
        <img src="/images/bg.jpg" alt="Ebony Bruce Travels Background" className="w-full h-full object-cover"/>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative z-20 animate-in fade-in zoom-in-95 delay-300 duration-700 mb-4 md:mb-6">
          <SearchBox onSearch={onSearch} loading={loading} activeTab={activeSearchTab} onTabChange={onTabChange}/>
        </div>
        
        
        {(isHotelTab() || isCarTab()) && (<div className="mt-8 md:mt-12 animate-in fade-in delay-500 duration-1000">
            ...
            <div className="flex flex-wrap justify-center gap-4 md:gap-6">
              {(isHotelTab() ? hotelDestinations : carRentalLocations).map((item, index) => (<button key={item.code} onClick={() => isHotelTab()
                    ? handleQuickHotelSearch(item)
                    : handleQuickCarSearch(item)} className="group relative overflow-hidden rounded-xl w-28 h-28 md:w-32 md:h-32 flex-shrink-0 animate-in fade-in slide-in-from-bottom-4" style={{
                  ...
                  <div className={`absolute inset-0 ${isHotelTab()
                    ? 'bg-blue-500/0 group-hover:bg-blue-500/10'
                    : 'bg-green-500/0 group-hover:bg-green-500/10'} transition-all duration-300`}></div>
                </button>))}
            </div>
          </div>)}
