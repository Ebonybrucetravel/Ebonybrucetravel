    ...
    return (<section className="py-12 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex items-center justify-center space-x-8 md:space-x-12 lg:space-x-16 overflow-x-auto pb-4 hide-scrollbar">
          {partners.map((partner) => (<div key={partner.name} className="flex-shrink-0">
              <img src={partner.logo} alt={`${partner.name} logo`} className="h-10 md:h-12 w-auto object-contain transition-transform duration-300 hover:scale-110"/>
            </div>))}
        </div>
      </div>
