...
const ExclusiveOffers: React.FC<ExclusiveOffersProps> = ({ onTypeClick }) => {
    ...
    return (<section className="px-4 md:px-8 lg:px-16 py-12">
      <div className="flex justify-between items-center mb-8">
        ...
        <a href="#" className="font-semibold transition-colors duration-200 flex items-center gap-1 group" style={{ color: brandBlue }}>
          {t('View All') || 'View All'}
          <svg className="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke={brandBlue}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
        {offers.map((offer) => (<div key={offer.id} className="group cursor-pointer rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1" onClick={() => handleOfferClick(offer.type)}>
            <div className="relative h-48 md:h-56 overflow-hidden">
              <img src={offer.image} alt={offer.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy"/>
              {offer.badge && (<div className="absolute top-4 left-4 bg-red-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md">
                  {offer.badge}
                </div>)}
            </div>
            <div className="p-5 md:p-6">
              ...
              <button className="flex items-center gap-2 font-bold transition-all duration-300 group-hover:gap-3" style={{ color: brandBlue }} onMouseEnter={(e) => {
