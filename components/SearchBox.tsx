...
const SearchBox: React.FC<SearchBoxProps> = ({ onSearch, loading, activeTab: activeTabProp, onTabChange }) => {
    ...
    React.useEffect(() => {
        if (!carPickUpDate || !carDropOffDate || carPickUpDate !== carDropOffDate)
            return;
        const [ph, pm] = carPickUpTime.split(':').map(Number);
        const [dh, dm] = carDropOffTime.split(':').map(Number);
        const pickupMins = ph * 60 + (pm || 0);
        let dropoffMins = dh * 60 + (dm || 0);
        if (dropoffMins <= pickupMins)
            dropoffMins += 24 * 60;
        if (dropoffMins - pickupMins < 4 * 60) {
            const minDropoffMins = pickupMins + 4 * 60;
            const h = Math.floor(minDropoffMins / 60) % 24;
            const m = minDropoffMins % 60;
            setCarDropOffTime(`${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}`);
        }
    }, [carPickUpDate, carDropOffDate, carPickUpTime, carDropOffTime]);
    const getCityCode = (location: string): string => {
        ...
        if (codeMatch)
            return codeMatch[1];
        const popularDest = popularHotelDestinations.find(dest => location.toLowerCase().includes(dest.city.toLowerCase()) ||
            location.toLowerCase().includes(dest.name.toLowerCase()));
        if (popularDest)
            return popularDest.cityCode;
    ...
    const fetchCarLocationSuggestions = useCallback(async (query: string): Promise<CarLocationSuggestion[]> => {
        ...
        const filtered = airportsWithCities.filter(location => location.code.toLowerCase().includes(queryLower) ||
            location.city.toLowerCase().includes(queryLower) ||
            location.name.toLowerCase().includes(queryLower)).slice(0, 10);
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (result.success && Array.isArray(result.data)) {
                    ...
                    const uniqueSuggestions = suggestions.filter((airport, index, self) => index === self.findIndex((a) => a.code === airport.code && a.city === airport.city));
                    return uniqueSuggestions.slice(0, 12);
                }
            }
            const lowerQuery = query.toLowerCase();
            const filtered = popularAirports.filter(airport => airport.code.toLowerCase().includes(lowerQuery) ||
                airport.city.toLowerCase().includes(lowerQuery) ||
                airport.country.toLowerCase().includes(lowerQuery) ||
                airport.name.toLowerCase().includes(lowerQuery));
            const uniqueFiltered = filtered.filter((airport, index, self) => index === self.findIndex((a) => a.code === airport.code && a.city === airport.city));
    ...
    const fetchHotelLocationSuggestions = useCallback(async (query: string): Promise<HotelDestination[]> => {
        ...
        try {
            ...
            const lowerQuery = query.toLowerCase();
            const filtered = popularHotelDestinations.filter(dest => dest.city.toLowerCase().includes(lowerQuery) ||
    ...
    const extractAirportCode = (displayValue: string): string => {
        ...
        const popularMatch = popularAirports.find(airport => airport.city.toLowerCase().includes(lowerValue) ||
            lowerValue.includes(airport.city.toLowerCase()));
        if (popularMatch) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (activeTab === 'cars') {
            ...
            if (!pickUpCode || !dropOffCode || !carPickUpDate || !carDropOffDate) {
                toast.error('Please fill in all rental details: locations, and both date and time for pick-up and drop-off.');
                return;
            }
            const formatDT = (d: string, t: string) => {
                const date = new Date(d);
                const [h, m] = (t || '00:00').split(':');
                return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}T${String(Number(h) || 0).padStart(2, '0')}:${String(Number(m) || 0).padStart(2, '0')}:00`;
            };
            let pickupDt = formatDT(carPickUpDate, carPickUpTime);
            let dropoffDt = formatDT(carDropOffDate, carDropOffTime);
            const pickupMs = new Date(pickupDt).getTime();
            const dropoffMs = new Date(dropoffDt).getTime();
            const fourHoursMs = 4 * 60 * 60 * 1000;
            if (dropoffMs <= pickupMs) {
                toast.error('Drop-off must be after pickup. Use a later time on the same day (e.g. 6:00 PM) or a later date. Minimum rental is 4 hours.');
                return;
            }
            if (dropoffMs - pickupMs < fourHoursMs) {
                toast.error('Drop-off must be at least 4 hours after pickup. Please choose a later drop-off time or date.');
                return;
            }
            onSearch({
                type: 'car-rentals',
                ...
                currency: 'GBP',
                carPickUp: carPickUp || undefined,
                carDropOff: carDropOff || undefined,
            });
        }
            ...
            const flightSegments = segments.map((segment, index) => {
                ...
                if (!fromCode)
                    errors.push(`Segment ${index + 1}: Invalid departure location`);
                if (!toCode)
                    errors.push(`Segment ${index + 1}: Invalid arrival location`);
                if (!segment.date)
    ...
    const renderTravellerDropdown = () => (showTravellerDropdown && (<div className="absolute top-full right-0 mt-2 w-72 md:w-80 bg-white rounded-xl shadow-2xl border border-gray-100 p-5 z-50 animate-in fade-in slide-in-from-top-2" onClick={(e) => e.stopPropagation()}>
    ...
    const renderCarTravellerDropdown = () => (showCarTravellerDropdown && (<div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 p-5 z-50 animate-in fade-in slide-in-from-top-2" onClick={(e) => e.stopPropagation()}>
        <div className="space-y-4">
          <div className="flex justify-between items-center">
    ...
    const renderRoomDropdown = () => (showRoomDropdown && (<div className="absolute top-full left-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 p-5 z-50 animate-in fade-in slide-in-from-top-2" onClick={(e) => e.stopPropagation()}>
        <div className="space-y-4">
          <div className="flex justify-between items-center">
    ...
    const renderAirportDropdown = (suggestions: Airport[], type: 'from' | 'to', index: number = 0) => (<div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-2xl border border-gray-200 max-h-60 overflow-y-auto z-50">
      {loadingSuggestions ? (<div className="px-4 py-3 text-center text-gray-500">
        ...
        </div>) : suggestions.length === 0 ? (<div className="px-4 py-3 text-center text-gray-500 text-sm">
          ...
          {suggestions.map((airport, idx) => (<button key={`${type}-${airport.code}-${index}-${airport.city}-${idx}-${airport.name}`} type="button" className="w-full text-left px-4 py-3 hover:bg-blue-50 transition-colors border-b border-gray-100 last:border-b-0 group" onClick={() => handleAirportSelect(airport, type, index)}>
    ...
    const renderHotelLocationDropdown = () => (showHotelLocationDropdown && (<div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-2xl border border-gray-200 max-h-60 overflow-y-auto z-50">
        {loadingHotelSuggestions ? (<div className="px-4 py-3 text-center text-gray-500">
            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-[#33a8da] mx-auto"></div>
            <p className="text-xs mt-2">Searching destinations...</p>
          </div>) : hotelLocationSuggestions.length === 0 ? (<div className="px-4 py-3 text-center text-gray-500 text-sm">No destinations found. Try a different search.</div>) : (<>
            <div className="px-4 py-2 bg-gray-50 border-b border-gray-100">
              <div className="text-xs font-bold text-gray-500">POPULAR DESTINATIONS</div>
            </div>
            {hotelLocationSuggestions.map((dest) => (<button key={`hotel-${dest.cityCode}`} type="button" className="w-full text-left px-4 py-3 hover:bg-blue-50 transition-colors border-b border-gray-100 last:border-b-0" onClick={() => handleHotelDestinationSelect(dest)}>
                <div className="flex items-start gap-3">
                  {dest.image && (<div className="flex-shrink-0">
                      <img src={dest.image} alt={dest.city} className="w-10 h-10 rounded-lg object-cover"/>
                    </div>)}
                  <div className="flex-1 min-w-0">
            ...
            {hotelLocationSuggestions.length >= 6 && (<div className="px-4 py-3 bg-gray-50 border-t border-gray-100">
                <div className="text-xs text-center text-gray-500">
    ...
    const renderCarLocationDropdown = (suggestions: CarLocationSuggestion[], type: 'pickUp' | 'dropOff') => (<div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-2xl border border-gray-200 max-h-60 overflow-y-auto z-50">
      {suggestions.map((location, idx) => (<button key={`${location.code}-${idx}`} type="button" className="w-full text-left px-4 py-3 hover:bg-blue-50 transition-colors border-b border-gray-100 last:border-b-0 group" onClick={() => handleCarLocationSelect(location, type)}>
          <div className="flex items-start gap-3">
    ...
    const renderFlightSegmentRow = (segment: Segment, index: number) => {
        ...
        return (<div key={index} className="bg-[#33a8da] rounded-[12px] p-1 flex flex-col lg:flex-row items-stretch gap-1 mb-2 shadow-sm animate-in slide-in-from-left duration-200">
            ...
            <div className="relative" ref={index === activeSegmentIndex ? fromRef : null}>
              <div className="bg-white p-3 md:p-4 flex items-center gap-3 relative rounded-t-lg sm:rounded-l-lg sm:rounded-tr-none">
                ...
                <div className="flex-1 relative">
                  ...
                  <input type="text" value={segment.from} onChange={(e) => handleFromInputChange(e.target.value, index)} onFocus={() => {
                setActiveSegmentIndex(index);
                ...
                setShowFromDropdown(true);
                setShowToDropdown(false);
            }} className="w-full font-bold text-gray-900 focus:outline-none text-base bg-transparent p-0 pr-6" placeholder="City or airport code"/>
                  {segment.from && (<button type="button" onClick={() => {
                    const newSegments = [...segments];
            ...
            <div className="relative" ref={index === activeSegmentIndex ? toRef : null}>
              <div className="bg-white p-3 md:p-4 flex items-center gap-3 border-t sm:border-t-0 sm:border-l border-gray-100 rounded-b-lg sm:rounded-r-lg sm:rounded-bl-none">
                ...
                <div className="flex-1 relative">
                  ...
                  <input type="text" value={segment.to} onChange={(e) => handleToInputChange(e.target.value, index)} onFocus={() => {
                setActiveSegmentIndex(index);
                ...
                setShowFromDropdown(false);
                  ...
                  {segment.to && (<button type="button" onClick={() => {
                    const newSegments = [...segments];
          ...
          <div className="lg:col-span-6 grid grid-cols-1 sm:grid-cols-2 gap-1">
            <div className={`p-3 md:p-4 flex items-center gap-3 relative cursor-pointer group hover:bg-gray-50 transition border-t sm:border-t-0 lg:border-l border-gray-100 ${isRangeSelected ? 'bg-blue-50/50' : 'bg-white'}`}>
              ...
              <div className="flex-1 flex gap-2 min-w-0">
                ...
                {tripType === 'round-trip' && index === 0 && (<div className="flex-1 min-w-0 border-l border-gray-100 pl-2 relative h-10 flex flex-col justify-center">
            ...
            <div className="bg-white p-3 md:p-4 flex items-center gap-3 relative cursor-pointer group hover:bg-gray-50 transition border-t sm:border-t-0 sm:border-l border-gray-100 rounded-b-lg lg:rounded-r-lg" ref={isFirst ? travellerRef : null} onClick={() => isFirst && setShowTravellerDropdown(!showTravellerDropdown)}>
              ...
              {isMultiCity && index > 0 && (<button type="button" onClick={(e) => { e.stopPropagation(); removeSegment(index); }} className="absolute -right-3 -top-3 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center hover:bg-red-600 transition shadow-md z-30">
    ...
    return (<div className="w-full max-w-7xl mx-auto px-2 md:px-4">
      <div className="bg-white rounded-2xl md:rounded-[24px] shadow-2xl overflow-visible">
        ...
        ].map((tab) => (<button key={tab.id} type="button" onClick={() => handleLocalTabChange(tab.id)} className={`flex items-center gap-2 pb-3 md:pb-5 transition-all relative shrink-0 ${activeTab === tab.id ? 'text-[#33a8da]' : 'text-gray-400 hover:text-blue-500 font-bold'}`}>
        ...
        <form onSubmit={handleSubmit} className="p-4 md:p-6 lg:p-8">
          {activeTab === 'flights' && (<div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-2">
                <div className="flex flex-wrap items-center gap-4 md:gap-6">
                  {(['round-trip', 'one-way', 'multi-city'] as const).map((opt) => (<button key={opt} type="button" onClick={() => handleTripTypeChange(opt)} className="flex items-center gap-2 group">
                  ...
                  <div className="flex items-center gap-2 md:gap-4 border-l border-gray-100 pl-4 h-8">
                    <div className="relative" ref={cabinRef}>
                      ...
                      {showCabinDropdown && (<div className="absolute top-full left-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50">
                          {[
                    { label: 'Economy', value: 'economy' },
                    { label: 'Premium Economy', value: 'premium_economy' },
                    { label: 'Business', value: 'business' },
                    { label: 'First Class', value: 'first' }
                ].map((cls) => (<button key={cls.value} type="button" onClick={() => {
                    ...
                    <div className="relative" ref={filtersRef}>
                      <button type="button" onClick={() => setShowFiltersDropdown(!showFiltersDropdown)} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-[10px] md:text-xs font-bold transition-all ${showFiltersDropdown ? 'border-[#33a8da] bg-blue-50 text-[#33a8da]' : 'border-gray-100 bg-gray-50 text-gray-700 hover:border-gray-300'}`}>
                      ...
                      {showFiltersDropdown && (<div className="absolute top-full right-0 lg:left-0 mt-2 w-64 md:w-72 bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 z-50 animate-in fade-in slide-in-from-top-2">
                          <div className="space-y-6">
                            <div>
                              <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Stops</h4>
                              <div className="grid grid-cols-2 gap-2">
                                {['Any', 'Non-stop', '1 Stop', '2+ Stops'].map((stop) => (<button key={stop} type="button" onClick={() => setStopsFilter(stop)} className={`py-2 px-3 rounded-lg text-[10px] md:text-xs font-bold border transition-all ${stopsFilter === stop ? 'bg-[#33a8da] text-white border-[#33a8da]' : 'bg-gray-50 text-gray-600 border-gray-100 hover:border-gray-200'}`}>
                                    {stop}
                                  </button>))}
                              </div>
                            </div>
                            <div>
                              ...
                              <input type="range" min="100" max="10000" step="100" value={maxPrice} onChange={(e) => setMaxPrice(parseInt(e.target.value))} className="w-full h-1.5 bg-gray-100 rounded-lg appearance-none cursor-pointer accent-[#33a8da]"/>
                            </div>
                            <button type="button" onClick={() => setShowFiltersDropdown(false)} className="w-full py-2.5 bg-gray-900 text-white text-[10px] md:text-xs font-bold rounded-xl hover:bg-black transition-colors uppercase tracking-widest">
                              Apply
                            </button>
                          </div>
                        </div>)}
              ...
              <div className="space-y-1">
                ...
                {tripType === 'multi-city' && segments.length < 4 && (<button type="button" onClick={addSegment} className="flex items-center gap-2 text-[#33a8da] font-bold text-xs bg-blue-50 px-4 py-2 rounded-xl hover:bg-blue-100 transition shadow-sm border border-blue-100">
          ...
          {activeTab === 'hotels' && (<div className="flex flex-col lg:flex-row items-stretch gap-1 bg-[#33a8da] rounded-xl p-1">
                ...
                <div className="md:col-span-4 relative" ref={hotelLocationRef}>
                  <div className="bg-white p-3 md:p-4 flex items-center gap-3 rounded-t-lg md:rounded-l-lg md:rounded-tr-none">
                    ...
                    <div className="flex-1 relative">
                      ...
                      <input type="text" value={hotelLocation} onChange={(e) => handleHotelLocationChange(e.target.value)} onFocus={() => {
                if (hotelLocation.length < 2) {
                    setHotelLocationSuggestions(popularHotelDestinations.slice(0, 6));
                }
                setShowHotelLocationDropdown(true);
            }} className="w-full font-bold text-gray-900 focus:outline-none text-sm md:text-base bg-transparent p-0 pr-6" placeholder="City, country, or landmark"/>
                      {hotelLocation && (<button type="button" onClick={() => {
                    setHotelLocation('');
                    setShowHotelLocationDropdown(false);
                }} className="absolute right-0 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-lg">
                          ×
                ...
                <div className="md:col-span-4 bg-white p-3 md:p-4 flex items-center gap-3 relative border-t md:border-t-0 md:border-l border-gray-100">
                  ...
                  <div className="flex-1 grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input type="date" min={today} value={checkInDate} onChange={(e) => setCheckInDate(e.target.value)} className="w-full font-bold outline-none text-xs bg-transparent p-0"/>
                    </div>
                    <div>
                      <label className="block text-[8px] font-bold text-gray-400 uppercase mb-0.5">Out</label>
                      <input type="date" min={checkInDate || today} value={checkOutDate} onChange={(e) => setCheckOutDate(e.target.value)} className="w-full font-bold outline-none text-xs bg-transparent p-0"/>
                    </div>
                  </div>
                </div>
  ...
  {loading ? (
    <div className="flex items-center gap-2">
      ...
      <span>...</span>
          ...
          {activeTab === 'cars' && (<div className="flex flex-col lg:flex-row items-stretch gap-1 bg-[#33a8da] rounded-xl p-1">
              <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-1">
                ...
                <div className="md:col-span-3 relative" ref={carPickUpRef}>
                  <div className="bg-white p-3 md:p-4 rounded-t-lg md:rounded-l-lg">
                    ...
                    <input type="text" value={carPickUp} onChange={(e) => handleCarPickUpChange(e.target.value)} onFocus={() => setShowCarPickUpDropdown(true)} placeholder="City or Airport" className="w-full font-bold focus:outline-none bg-transparent p-0"/>
                  </div>
                ...
                <div className="md:col-span-3 relative" ref={carDropOffRef}>
                  <div className="bg-white p-3 md:p-4 border-t md:border-t-0 md:border-l border-gray-100">
                    ...
                    <input type="text" value={carDropOff} onChange={(e) => handleCarDropOffChange(e.target.value)} onFocus={() => setShowCarDropOffDropdown(true)} placeholder="City or Airport (same for round-trip)" className="w-full font-bold focus:outline-none bg-transparent p-0"/>
                  </div>
                ...
                <div className="md:col-span-3 bg-white p-3 md:p-4 border-t md:border-t-0 md:border-l border-gray-100 grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input type="date" min={today} value={carPickUpDate} onChange={(e) => setCarPickUpDate(e.target.value)} className="w-full font-bold outline-none text-xs bg-transparent p-0"/>
                  </div>
                  <div>
                    <span className="block text-[9px] font-bold text-gray-400 uppercase">Pick-up Time</span>
                    <input type="time" value={carPickUpTime} onChange={(e) => setCarPickUpTime(e.target.value)} className="w-full font-bold outline-none text-xs bg-transparent p-0"/>
                  </div>
                  <div>
                    <span className="block text-[9px] font-bold text-gray-400 uppercase">Drop-off Date</span>
                    <input type="date" min={carPickUpDate || today} value={carDropOffDate} onChange={(e) => setCarDropOffDate(e.target.value)} className="w-full font-bold outline-none text-xs bg-transparent p-0"/>
                  </div>
                  <div>
                    <span className="block text-[9px] font-bold text-gray-400 uppercase">Drop-off Time</span>
                    <input type="time" value={carDropOffTime} onChange={(e) => setCarDropOffTime(e.target.value)} className="w-full font-bold outline-none text-xs bg-transparent p-0"/>
                  </div>
                </div>
...
>
  {loading ? (
    <div className="flex items-center gap-2">
      ...
      {showCarTravellerDropdown && (<div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm" onClick={() => setShowCarTravellerDropdown(false)}>
