...
const Navbar: React.FC<NavbarProps> = ({
  ...
  const selectedCurrency =
    currencies.find((c) => c.code === currencyCode) || currencies[1];
  ...
  const handleLanguageSelect = (lang: Language) => {
    ...
    const matchingCurrency = currencies.find(
      (c) => c.code === lang.defaultCurrencyCode,
    );
    if (matchingCurrency) {
      setCurrency({
    ...
    <nav className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20 gap-4">
          ...
          {/* Desktop Navigation */}
          <div
            className={`hidden lg:flex items-center space-x-8 ${!isLoggedIn ? "flex-1 justify-center" : ""}`}
          >
              ...
              >
                ...
                <button
                  ...
                  className={`w-11 h-11 rounded-full bg-gradient-to-br from-[#32A6D7] to-[#2c95c2] flex items-center justify-center border-2 shadow-md hover:scale-105 transition-all overflow-hidden ${
                    isProfileMenuOpen
                      ? "border-blue-400 scale-105 ring-4 ring-blue-50"
                      : "border-white"
                  }`}
                    ...
                    <div className="p-2 max-h-[440px] overflow-y-auto hide-scrollbar space-y-0.5">
                      {profileMenuItems.map((item) => (
                        <button
                          key={item.id}
                          onClick={() =>
                            handleProfileDropdownItemClick(item.id)
                          }
