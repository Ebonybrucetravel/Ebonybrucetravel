...
const SearchFilters: React.FC<SearchFiltersProps> = ({ type, results, searchParams, onApplyFilters, onResetFilters }) => {
    ...
    const renderFlightFilters = () => (<div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          <input type="range" min={availableOptions.price?.min || 0} max={availableOptions.price?.max || 100000} value={activeFilters.priceRange?.max || 100000} onChange={(e) => handleFilterChange('priceRange', {
      ...
      {availableOptions.airlines && availableOptions.airlines.length > 0 && (<div>
          <h3 className="font-medium text-gray-900 mb-3">Airlines</h3>
          <div className="space-y-2">
            {availableOptions.airlines.slice(0, 5).map((airline: string) => (<label key={airline} className="flex items-center">
                <input type="checkbox" checked={activeFilters.airlines?.includes(airline)} onChange={(e) => {
      ...
      <div>
        ...
        <div className="space-y-2">
          {[0, 1, 2, 3].map((stops) => (<label key={stops} className="flex items-center">
              <input type="radio" name="maxStops" checked={activeFilters.maxStops === stops} onChange={() => handleFilterChange('maxStops', stops)} className="w-4 h-4 text-blue-600"/>
      ...
      <div>
        ...
        <div className="space-y-2">
          {['Economy', 'Premium Economy', 'Business', 'First'].map((cabin) => (<label key={cabin} className="flex items-center">
              <input type="checkbox" checked={activeFilters.cabinClass?.includes(cabin)} onChange={(e) => {
    ...
    const renderHotelFilters = () => (<div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          {[5, 4, 3, 2, 1].map((stars) => (<label key={stars} className="flex items-center">
              <input type="checkbox" checked={activeFilters.starRating?.includes(stars)} onChange={(e) => {
      ...
      {availableOptions.rating && (<div>
          ...
          <div className="space-y-2">
            ...
            <input type="range" min="0" max="5" step="0.5" value={activeFilters.guestRating?.max || 5} onChange={(e) => handleFilterChange('guestRating', {
                min: 0,
      ...
      <div>
        ...
        <div className="space-y-2">
          {['Free WiFi', 'Swimming Pool', 'Spa', 'Restaurant', 'Fitness Center', 'Parking'].map((amenity) => (<label key={amenity} className="flex items-center">
              <input type="checkbox" checked={activeFilters.amenities?.includes(amenity)} onChange={(e) => {
    ...
    const renderCarRentalFilters = () => (<div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          {['SUV', 'Sedan', 'Van', 'Convertible', 'Luxury', 'Economy'].map((type) => (<label key={type} className="flex items-center">
              <input type="checkbox" checked={activeFilters.vehicleType?.includes(type)} onChange={(e) => {
      ...
      <div>
        ...
        <div className="space-y-2">
          {['Automatic', 'Manual'].map((transmission) => (<label key={transmission} className="flex items-center">
              <input type="checkbox" checked={activeFilters.transmission?.includes(transmission)} onChange={(e) => {
      ...
      <div>
        ...
        <div className="space-y-2">
          {[2, 4, 5, 7, 8].map((seats) => (<label key={seats} className="flex items-center">
              <input type="checkbox" checked={activeFilters.seats?.includes(seats)} onChange={(e) => {
    ...
    const countActiveFilters = () => {
        ...
        if (type === 'flights') {
            ...
            if (activeFilters.priceRange?.max !== availableOptions.price?.max)
                count += 1;
        }
        else if (type === 'hotels') {
            ...
            if (activeFilters.priceRange?.max !== availableOptions.price?.max)
                count += 1;
            if (activeFilters.guestRating?.max !== availableOptions.rating?.max)
                count += 1;
        }
        else if (type === 'car-rentals') {
    ...
    return (<>
      ...
      <button onClick={() => setIsOpen(!isOpen)} className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">
        <svg className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/>
        </svg>
        <span className="font-medium text-gray-700">Filters</span>
        {filterCount > 0 && (<span className="bg-blue-600 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
            {filterCount}
          </span>)}
      </button>

      
      {isOpen && (<div className="fixed inset-0 z-50 lg:relative lg:inset-auto">
          ...
          <div className="fixed inset-y-0 left-0 w-full max-w-sm bg-white shadow-2xl overflow-y-auto lg:relative lg:inset-auto lg:shadow-lg lg:rounded-2xl lg:border lg:border-gray-200">
            <div className="p-6">
              ...
              <div className="mt-8 pt-6 border-t border-gray-200 space-y-3">
                <button onClick={handleApply} className="w-full py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold rounded-xl shadow-lg hover:from-blue-600 hover:to-blue-700 transition">
                  Apply Filters
                </button>
                <button onClick={handleReset} className="w-full py-2.5 border border-gray-300 text-gray-700 font-medium rounded-xl hover:bg-gray-50 transition">
                  Reset All
                </button>
              </div>
            </div>
