...
interface FooterProps {
  onLogoClick?: () => void; // Optional custom handler (e.g. analytics)
  setIsMobileMenuOpen?: (open: boolean) => void;
  onAdminClick?: () => void; // ← Added admin click handler
}
...
const Footer: React.FC<FooterProps> = ({
  ...
  const handleLogoClick = () => {
    // Call any custom logic (e.g. tracking, closing menu)
    onLogoClick?.();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-20">
          ...
          <div className="md:col-span-4">
            {/* Clickable Logo → Always goes to homepage */}
            <div
            ...
            >
              <Image
                ...
                height={40}
                priority
                className="h-12 w-auto" // Slightly larger as requested
              />
            </div>
                ...
                <ul className="space-y-4">
                  {cat.links.map((link, index) => (
                    <li key={index}>
                      ...
                      >
                        <svg
                          className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-blue-600"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
