...
const BookingFailed: React.FC<BookingFailedProps> = ({
  item,
  searchParams,
  onBack,
  onRetry,
  ...
  const travelersCount = parseInt(
    searchParams?.travellers?.match(/\d+/)
      ? searchParams.travellers.match(/\d+/)[0]
      : "1"
  );
  const destination =
  ...
  const totalPrice =
    subtotal +
    (isHotel ? cleaningFee * (currency.code === "USD" ? 1 : 1500) : 0);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-[16px] border border-gray-100 shadow-sm p-10">
        ...
        <div className="flex items-center gap-3 text-gray-400 pt-8 border-t border-gray-50">
          ...
          <span className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">
            {travelersCount} Adults, 0 Children
          </span>
        </div>
      </div>
      ...
      <div className="bg-[#f0f7ff] rounded-[16px] border border-[#dbeafe] p-8 flex items-start gap-4">
        ...
        <div className="space-y-2">
          <h4 className="text-sm font-black text-[#1e40af] uppercase tracking-tight">
            Check-in Instruction
          </h4>
          <p className="text-[13px] font-bold text-[#1e40af]/80 leading-relaxed">
            The front desk is open 24/7. Please have your confirmation ID and a
  ...
  const renderFlightDetails = () => (
    <div className="bg-white rounded-[16px] border border-gray-100 shadow-sm p-10">
      ...
      <p className="text-sm font-bold text-gray-400 italic">
        Itinerary preview unavailable for failed bookings. Please try again.
      </p>
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-12">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-black text-red-500 uppercase tracking-tight">
                Booking Unsuccessful
              </h1>
            </div>
            <p className="text-lg md:text-xl font-bold text-red-500 max-w-xl">
              Sorry, but we are unable to complete your booking at this time.
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <button
                key={btn.label}
                className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-100 rounded-xl text-[11px] font-black text-gray-600 hover:bg-gray-50 transition shadow-sm active:scale-95 uppercase tracking-tight"
              >
                <svg
          ...
          <aside className="w-full lg:w-[460px]">
            <div className="bg-white rounded-[24px] shadow-lg border border-gray-100 overflow-hidden">
              ...
              <div className="p-10 space-y-8">
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center text-base">
                    <span className="text-gray-600 font-bold">
                      {isHotel
                        ...
                        : `${unitCount} travelers x ${
                            currency.symbol
                          }${numericPrice.toLocaleString()}`}
                    </span>
                    <span
                      className={`font-black ${
                        isCar ? "text-purple-600" : "text-[#33a8da]"
                      }`}
                    >
                      {currency.symbol}
                  ...
                  {isHotel && (
                    <div className="flex justify-between items-center text-base">
                      ...
                      <span className="text-[#33a8da] font-black">
                        {currency.symbol}
                        {(
                          cleaningFee * (currency.code === "USD" ? 1 : 1500)
                        ).toLocaleString()}
                        .00
                      </span>
                    </div>
                ...
                <button
                  onClick={onRetry}
                  className={`w-full text-white font-black py-6 rounded-2xl shadow-2xl transition transform active:scale-95 text-lg mt-6 ${
                    isCar
                      ? "bg-purple-600 hover:bg-purple-700 shadow-purple-100"
                      : "bg-[#33a8da] hover:bg-[#2c98c7] shadow-blue-100"
                  }`}
                >
                  Manage Booking
                </button>
        ...
        <div className="mt-16 bg-white rounded-[24px] p-10 border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-8 shadow-sm">
          <div className="flex items-center gap-8">
            <div
              className={`w-16 h-16 rounded-full flex items-center justify-center text-white shadow-xl shrink-0 ${
                isHotel
                  ? "bg-[#33a8da] shadow-blue-100"
                  : isCar
                  ? "bg-[#33a8da] shadow-blue-100"
            ...
            <div>
              <h4 className="text-lg font-black text-gray-900 tracking-tight leading-tight mb-1">
                {isFlight
                  ? "Need a Hotel for your stay?"
                  : "Need a Flight for your trip?"}
              </h4>
              <p className="text-sm font-bold text-gray-400">
                Experience the perfect blend of comfort and affordability
              </p>
            </div>
          </div>
          <button
